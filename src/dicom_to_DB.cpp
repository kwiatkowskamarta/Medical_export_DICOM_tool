...
// liczenie wieku z daty urodzenia
int calculateAgeFromBirthYear(const string& birth_date_str) {
    if (birth_date_str.length() < 4) return -1; // nieprawidlowy format
...
// wstawianie danych do tabeli 'images'
...
// wstawianie danych do tabeli 'series'
void insert_series_to_db(SQLHDBC hdbc, const string& study_uid, const dicomhero::DataSet& dcm) {
    ...
    // sprawdzenie czy seria juz istnieje w bazie
    SQLHSTMT hstmt;
    SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
...
// wstawianie danych do tabeli 'studies'
void insert_study_to_db(SQLHDBC hdbc, const string& patient_id, const dicomhero::DataSet& dcm) {
    ...
    // sprawdzenie czy badanie juz istnieje
    SQLHSTMT hstmt;
    SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
...
// wstawianie pacjentow do bazy danych
void insert_dicom_to_db(SQLHDBC hdbc, const string& filepath) {
    ...
    string name = converter.to_bytes(dcm.getUnicodeString(dicomhero::TagId(dicomhero::tagId_t::PatientName_0010_0010), 0));
        ...
        // jesli brakuje patient_id - autouzupelnianie
        if (patient_id.empty()) {
    ...
    // jesli brak pola 'age', to obliczane jest ono z daty urodzenia
    int age = -1;
    try {
    ...
    // sprawdzenie czy pacjent juz istnieje
    SQLHSTMT hstmt;
    SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
